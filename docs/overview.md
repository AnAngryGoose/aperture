...
**v0.4.1-alpha.2** — Dashboard cleanup pass on top of the v0.4.1 monitoring rewrite. **Needs Attention** panel between the page header and the host grid groups every active issue (offline hosts, unhealthy containers, metric breaches, open alerts) into a single scannable list — critical first, then warning, top-5 visible with "View all issues" linking to /alerts when more exist. Each row shows host name + severity + reason + metric value (CPU %, °C, alert title, etc.) and clicks through to the relevant host-detail tab via URL hash (e.g. `#cpu`, `#events`). New shared `lib/monitoring/issues.ts` module backs both the panel and the PageHeader summary strip so the OPEN ALERTS count and the alert rows literally come from the same array (fixes a class of "count says 0 but the list shows 1" drift). RichCard prototype-parity overhaul: status label text next to the dot, consolidated mono meta line (OS + arch + CPU model + cores + RAM + uptime), per-metric sub-text (cores + max temp for CPU, used/total for memory, ↑/↓ pair for net, mount info for disk), DISK rendered as a meter bar, side panel now stacks the Containers summary + a TOP BY CPU process list. Fixed an infinite reactive loop on the host detail page that was firing ~120 GET/sec when the bundle effect read its own output. Six legacy `host.open_alerts` readers (Sidebar badge, FilterBar, HostGrid filter, RichCard footer, TileCard pill, /hosts page) were silently returning 0 after C3's data-model split; monitoringStore now mirrors both fields so the existing readers light back up. **v0.4.1-alpha.1** — Monitoring rewrite (Beszel parity): per-host monitoring config persisted server-side (sample interval, enabled families, NIC/sensor/mount filters, mem_calc, warn/crit thresholds, retention) and pushed to agents at connect + on edit. New SQLite tables for temp/CPU-core/process/container history; SQL helpers and aggregated `/api/monitoring/{overview,bundle,catalog}` endpoints replace the dashboard's N+1 fan-out. Typed SSE v2 envelope (`metric`/`host_status`/`container_summary`/`alert`) with backwards-compat flat fields. Host detail page rewritten as 10-tab v0.4 surface (Overview / CPU / Memory / Disk / Network / Sensors / Processes / Docker / Events / Settings) driven by a single bundle fetch + 30s reconciliation. DrillIn modal gets live sensor panel, top-5 process peek, full-monitoring CTA. Dashboard cards now configurable (2-4 metrics per host via metric catalog). Global monitoring defaults editable from Settings. Alerts: extended dotted-target vocabulary (`iface.eth0.rx_rate`, `mount./.pct`, `temp.cpu.value`, `proc.nginx.cpu_pct`, `host.status`), two-step rule editor (category → target → leaf), Beszel-style rule templates, single Shoutrrr-routed channel dispatcher unlocks 16+ notification services (Telegram, Matrix, Pushover, Teams, etc.). Offline watchdog fires status alerts when a host stops sending samples. Efficiency wins: heap-based top-N processes, alert-rule index + in-memory cache, mount-list cache, container-summary cache, per-host retention loop. **v0.4.0-alpha.2** — Full UI redesign: sidebar shell, Geist font, dark/light/system theme with user-selectable accent colors, three-variant host dashboard (Rich/Tile/List), live SSE sparklines, host drill-in slide-over panel, AddHost modal (agent/docker/SSH), Settings → Appearance, stub routes for all nav destinations. **v0.3.0-alpha.4** — Wave 2: single-admin session auth, bcrypt password, HttpOnly session cookies, first-run setup page, login page, change-password UI, per-channel 15s notification timeout, build-tag CORS split. **v0.3.0-alpha.3** — Wave 1 correctness: fixed silent alert evaluation failure (missing SQL column), fixed terminal JSON double-encoding, fixed resolve timestamp drift, added `TerminalProvider` interface and `internal/agentproto` shared frame types. **v0.3.0-alpha.2** — Phase 3 kickoff: deep Docker network management. **v0.3.0-alpha.1** — Phase 2 complete: docker-compose stack management directly from the UI. Discovers all existing compose stacks automatically (no ownership, no migration), shows live service state per stack, and supports the full lifecycle (up, down, restart, pull, stop, start) at both stack and individual-service granularity. Per-stack compose file viewer/editor with Save + Deploy in one click. Aggregated and per-service log tailing. Create new stacks from scratch with a YAML editor and optional immediate start. Works identically for the local hub host and remote agent hosts. **v0.2.0-alpha.5** — Phase 1 complete: remote agent transport, token-based auth, exponential-backoff reconnect, and the agent onboarding wizard. **v0.2.0-alpha.4** — UI/UX quality-of-life pass: host status pills, per-host alert badges, stale/offline banners, quick container search, ESC-to-close, dynamic page titles, absolute-timestamp tooltips, global toast system. **v0.2.0-alpha.3** added chart UX improvements and extensible alert notification channels: Discord, Slack, ntfy, Gotify, and generic webhook, each with per-rule severity levels and configurable resolve notifications. **v0.2.0-alpha.2** added full Beszel monitoring parity. **v0.2.0-alpha.1** added rich live monitoring depth and complete container lifecycle. **v0.1.0** established the base foundation.

- **UI (v0.4 + v0.4.1 host-detail rewrite):** Sidebar navigation shell (220px, collapsible planned). Dark/light/system theme toggle, six accent colors (teal default), all persisted in `localStorage`. Three host dashboard card variants — Rich (sparklines + side panel), Tile (compact 2×2 grid), List (row) — with a per-card widget picker that lets the user pick 2–4 metrics per host from a server-driven catalog. Host detail page is a 10-tab v0.4 surface (Overview / CPU / Memory / Disk / Network / Sensors / Processes / Docker / Events / Settings) driven by a single bundle fetch + 30s reconciliation. DrillIn slide-over shows BigMetric grid, live sensor panel, top-5 process peek, container counts, recent alerts, and a CTA into the full host page. AddHost modal with Install Agent / Docker API / SSH Probe methods. Settings → Appearance for theme/accent and a Monitoring defaults editor that drives any host without its own config. Live SSE at `/api/stream/metrics` carries typed envelopes (`metric` / `host_status` / `container_summary` / `alert`); pre-v0.5 clients still see flat fields under `type:"metric"`.
- Auto-discovers the local host, samples its system metrics every few seconds, and stores them in SQLite. Live snapshot is cached in-memory; richer per-entity history (`net_iface_metrics`, `disk_mount_metrics`, `disk_io_metrics`, `temp_metrics`, `cpu_core_metrics`, `process_metrics`, `container_metrics`) is persisted so the host-detail tabs can chart it. A `host_config` table holds per-host monitoring policy (sample interval, enabled families, NIC/sensor/mount filters, `mem_calc` mode, warn/crit thresholds, retention + per-table overrides). Edits push to the running collector / agent over the WebSocket transport — no agent restart needed.
- Lists all docker containers on the host with live CPU/memory stats. Per-host container summary is cached in the hub (15s ticker) and broadcast over SSE, replacing the dashboard's previous per-host Docker polling.
- Aggregated monitoring API: `/api/monitoring/{overview,bundle/{id},catalog}` give the frontend everything it needs in one fetch per surface, replacing the prior N+1 fan-out. Per-metric history endpoints (`temps`, `cpu`, `procs`, container) cover single-chart drill-ins.
- Web dashboard:
  - Host overview cards (CPU, memory, disk, container count) with configurable widgets, live SSE updates, click-to-open DrillIn.
  - Per-host detail page: 10 tabs covering aggregate CPU + per-core grid + per-core history; memory breakdown bar + history + swap; mount usage table + per-device disk I/O + per-mount history; per-interface table + aggregate + per-iface history; live sensor grid + multi-series temperature history; sortable process table with click-to-expand inline history charts; container summary + table; alert history; per-host monitoring config editor.
  - Container management: create (form: image, name, restart policy, env, ports, volumes, auto-start), deep inspect, live resource limit editing, recreate, start/stop/pause/unpause/restart/kill/remove, logs. Sorting + state filtering.
- **Alerts**: threshold rules on flat scalars (`cpu_pct`, `mem_pct`, `disk_pct`, `swap_pct`, `load_1/5/15`, `temp.max`) and dotted targets (`iface.<name>.{rx_rate,tx_rate,rx_bytes,tx_bytes}`, `mount.<path>.{pct,used,total}`, `temp.<sensor>.value`, `proc.<name>.{cpu_pct,mem_pct,mem_rss}`, `host.status`). Operators include `>`, `>=`, `<`, `<=`, `==`, `!=`. Status alerts fire via an offline watchdog when a host stops sending samples. Three predefined templates (Beszel defaults, Aggressive, Quiet) apply per-host or globally with one click. Per-rule severity, sustained-breach duration, auto-resolve, nav badge with firing count, host-detail Events tab for history.
- **Notification channels**: 17+ services via [Shoutrrr](https://containrrr.dev/shoutrrr/) (Discord, Slack, Telegram, Matrix, Pushover, Teams, ntfy, Gotify, Mattermost, Rocketchat, OpsGenie, IFTTT, Pushbullet, Bark, Join, Lark, WeCom, Zulip) routed through a single dispatcher, plus a generic webhook for custom integrations. Legacy discord/slack/ntfy/gotify channel rows are URL-translated to Shoutrrr at send time — no DB migration needed. Per-channel minimum-severity filter, resolve-notify toggle, and Test button.
