...
### Added — Backend foundation (Compartment 1)

- **`internal/store/schema.sql`** — five new tables: `temp_metrics`, `cpu_core_metrics`, `process_metrics`, `container_metrics`, `host_config`. Persist what used to be live-only fields so the UI can chart history for each. `host_config` stores per-host monitoring policy (sample interval, enabled families, JSON filters, mem_calc, retention + per-table overrides, warn/crit thresholds for cpu/mem/disk/temp). Hot-path index `idx_alert_rules_eval` for the evaluator's per-sample rule lookup. **Why:** the live-only model meant graphs couldn't go beyond what fit in RAM; persisting per-entity history is the precondition for the host-detail tabs and any "yesterday this happened" investigation.
- **`internal/store/store.go`** — `Insert{Temps,CPUCores,ProcessSnapshot,ContainerMetrics}` + `{Temp,CPUCore,Process,ContainerMetric}Range` helpers, `HostConfig` CRUD with JSON serialization, `GetMonitoringDefaults`/`SetMonitoringDefaults` (user-customizable global defaults via `user_settings['monitoring.defaults']`), `PruneHostMetrics` for per-host scoped retention, `stridePicks` helper for in-Go downsampling shared across all range queries. `GetHostConfig` now overlays admin defaults onto the built-in fallback so a host without a row picks up admin-configured thresholds.
- **`internal/collector/collector.go`** — `ApplyConfig` plumbs per-host policy onto the local collector (enabled families, NIC/sensor/mount allow/deny filters, `mem_calc` mode, sample interval). Each family block in `sample()` is gated by `familyEnabled()`. Heap-based top-N processes (O(N log K) instead of two full sorts). 30s mount-list cache (was a per-tick `disk.PartitionsWithContext` syscall). **Why:** the agent now respects what the user wants monitored rather than blasting every category every tick; the heap + cache cut local-collector overhead noticeably on hosts with hundreds of processes or many mounts.
- **`internal/collector/families/`** (new subpackage) — `MetricFamily` interface + stub implementations for `smart`, `gpu`, `battery`, `systemd`. Returns `ErrNotImplemented` today but registered in the catalog as `experimental:true` so the UI can show them as known-but-disabled. **Why:** locks in the seam now so adding GPU monitoring (or any vendor-specific probe) is a single new file rather than a refactor.
- **`internal/alerts/alerts.go`** — in-memory rules cache invalidated on create/update/delete (avoids a SQLite query per ingested sample). Extended `MetricValue` parses dotted-target syntax: `iface.<name>.{rx_rate,tx_rate,rx_bytes,tx_bytes}`, `mount.<path>.{pct,used,total}`, `temp.<sensor>.value`, `proc.<name>.{cpu_pct,mem_pct,mem_rss}` (max across matching processes), plus `temp.max` (hottest sensor on the host). New `==` / `!=` ops for string-valued metrics. `host.status` resolves via a registered `StatusProvider` (Hub's `LatestStatus` lookup) and is encoded numerically (ok=0..offline=3) so the existing threshold pipeline works unchanged. New `EvaluateStatus(ctx, hostID, ts)` fires status rules on transitions independent of sample arrival. **Why:** Beszel-class alerting was the killer feature gap — without per-iface / per-mount targets and a host-offline rule, users couldn't migrate from Beszel without losing alerts.
- **`internal/hub/hub.go`** — typed SSE v2 envelope: `metric` / `host_status` / `container_summary` / `alert` events on the same stream, with backwards-compatible flat fields under `type:"metric"` so pre-v0.5 clients keep working. Status derivation moved server-side (was a client-side fallback) using each host's `host_config` thresholds. Per-host retention loop iterates hosts and prunes scoped to each. Container-summary loop polls registered docker providers every 15s and broadcasts on change — eliminates the dashboard's per-poll Docker round-trips. Offline watchdog flips stale hosts to "offline" every 30s and asks the evaluator to run host.status rules. `ConfigPusher` / `LocalApplier` seams route `host_config` edits to the right transport (local collector or remote agent). **Why:** moving status, container summary, and config dispatch into the hub means every client sees the same truth and the dashboard's polling load goes from O(hosts × poll interval) to O(1) initial fetch + SSE.
- **`internal/agentproto/frames.go`** — new `TypeConfig` frame pushes per-host monitoring policy from hub to agent. Agent's `runSession` handles incoming `config` frames and calls `col.ApplyConfig`. Hub pushes config at agent connect (post-ack) and on every `PUT /api/hosts/{id}/config`. **Why:** without this, remote agents were stuck at hub-startup defaults and edits in the UI never reached them.

### Added — Monitoring API spine (Compartment 2)

- **`internal/api/monitoring.go`** (new) — aggregated endpoints replacing the dashboard's prior N+1 fan-out and the host page's 7-call fan-out:
  - `GET /api/monitoring/overview` — hosts + latest sample + container counts + open-alert counts + status, all in one call.
  - `GET /api/monitoring/catalog` — single source of truth for families, scalar metrics, alert categories+leaves, rule templates. Used by the widget picker, alert rule editor, and host Settings tab.
  - `GET /api/hosts/{id}/monitoring/bundle?range=&points=&include=` — host record + latest sample + host_config + all history series for the range + open alerts. Bundle is what the host detail page reads on tab switch / range change.
  - `GET /api/hosts/{id}/config` + `PUT` — per-host monitoring policy; PUT pushes to the running collector/agent via the hub's `ConfigPusher`.
  - `GET/PUT /api/settings/monitoring-defaults` — global defaults editable from Settings, overlaid onto hosts without their own row.
  - `GET /api/hosts/{id}/metrics/{temps,cpu,procs}` and `GET /api/hosts/{id}/containers/{cid}/metrics` — per-metric history endpoints for single-chart drill-ins.

### Added — Frontend monitoring spine (Compartments 3–5)

- **`web/src/lib/stores/monitoring.svelte.ts`** (new, replaces the legacy host store via a thin re-export shim at `stores/hosts.svelte.ts`) — single overview fetch on mount, typed-envelope SSE consumer routes `metric` / `host_status` / `container_summary` / `alert` events to entries, 30s reconciliation poll as a safety net for missed SSE events. Server-supplied status preferred over fallback derivation. Sparkline ring buffers preserved across reconciliation reloads.
- **`web/src/lib/monitoring/metricCatalog.ts`** (new) — UI-side metric registry merged with the backend catalog. Drives the dashboard widget picker, the per-card metric rows, and the rule editor's scalar dropdown.
- **`web/src/routes/dashboard/+page.svelte`** — uses `monitoring.overview()` for first paint + SSE for live updates; poll cadence dropped from 5s to 30s reconciliation.
- **`web/src/routes/hosts/[id]/+page.svelte`** — full v0.4 rewrite. Tabbed surface (Overview / CPU / Memory / Disk / Network / Sensors / Processes / Docker / Events / Settings) driven by a single bundle fetch + 30s poll. Range picker persists per-host in localStorage. New components in `web/src/lib/components/host/`: `HostHeader`, `RangePicker`, plus 10 tab components (`OverviewTab` with 4 BigMetric cards + alert banner + top-process peek, `CPUTab` with aggregate + per-core grid + per-core history + load average, `MemoryTab` with used/cached/free segmented bar + swap + history, `DiskTab` with per-mount usage table + per-device I/O + per-mount charts, `NetworkTab` with per-iface table + aggregate rate + per-iface history, `SensorsTab` with live sensor grid + multi-series history, `ProcessesTab` with sortable table + click-to-expand inline history charts, `DockerTab` with container summary + table + link-out, `EventsTab` with alert history, `MonitoringSettingsTab` with config form for sample interval / families / thresholds / filters).
- **`web/src/lib/components/host/DrillIn.svelte`** — live max-temp BigMetric replaces the `—°C` placeholder, sensors mini-panel under the lower panels, top-5 by CPU peek for non-Docker hosts, "Open full host monitoring →" CTA into the new detail page.
- **`web/src/lib/components/dashboard/CardConfigModal.svelte`** (new) + **`CardMenu.svelte`** — per-card widget picker. Pick 2-4 metrics from the catalog (cpu, mem, swap, load_1/5, net_rx/tx_rate, disk_pct, temp_max, mem_used, uptime). Persisted in `dashboardLayout.cardWidgets` keyed by hostID; backend saves via the existing `settings/dashboard-layout` endpoint. `RichCard.svelte` now renders metric rows dynamically from the user's selection rather than hardcoded CPU/MEM/NET. **Why:** "very configurable, easily updated to allow for different things to be monitored" was an explicit goal — users now decide what each card surfaces without code changes.
- **`web/src/lib/components/primitives/Sparkline.svelte`** — interactive hover tooltip with value + timestamp; uses pointer events and an optional `xs` timestamp array.
- **`web/src/lib/Chart.svelte`** — restyled to v0.4 tokens (`--text-faint`, `--line`, `--bg-elev` resolved at construction time for canvas-friendly hex values), tooltip background uses theme tokens, legacy CSS aliases (`--border`, `--mono`) replaced.
- **`web/src/routes/settings/+page.svelte`** — new "Monitoring defaults" section: sample interval, retention days, mem_calc, status thresholds. Persisted via `/api/settings/monitoring-defaults`.

### Added — Alerts expansion (Compartment 6)

- **`internal/alerts/templates.go`** (new) — three predefined rule sets: "Beszel defaults" (CPU>90/60s warn, Mem>90/60s warn, Disk>85/300s warn, Temp>80/60s critical, host.status==offline/120s critical), "Aggressive" (lower thresholds, shorter durations), "Quiet" (higher thresholds, longer durations). `ApplyTemplate` skips duplicates by `(metric, op, threshold)` so re-apply is idempotent. `POST /api/alerts/templates/apply` exposes it.
- **`internal/alerts/ch_shoutrrr.go`** (new) — single dispatcher replacing the four hand-rolled channel files (`ch_discord.go`, `ch_slack.go`, `ch_ntfy.go`, `ch_gotify.go` — ~270 LOC deleted). New `shoutrrr` channel type accepts a raw URL for any of Shoutrrr's 16+ supported services (Telegram, Matrix, Pushover, Teams, Mattermost, Rocketchat, Bark, Pushbullet, OpsGenie, IFTTT, Join, Lark, WeCom, Zulip, Google Chat, etc.). Legacy types are URL-translated to Shoutrrr at send time — no DB migration needed, existing rows keep their original `type` + `config`. Webhook stays on its own simple sender (its JSON-POST body shape doesn't map cleanly onto Shoutrrr's `generic://`). **Why:** the duplicated channel files were a recurring maintenance burden and adding Telegram/Matrix/Pushover would have been three more files; routing through Shoutrrr unlocks them for free.
- **`web/src/routes/alerts/+page.svelte`** — "Apply template" panel with template + scope (global / per-host) selectors. Two-step rule editor replaces the single Metric dropdown: a Category select switches between simple scalar, per-iface, per-mount, per-sensor, per-process, and host-status modes; Target is a free-text input with hinted placeholders; Leaf is a dropdown sourced from the catalog. Host-status mode replaces threshold with a Status dropdown (ok/warn/crit/offline) and auto-sets `op === '=='` and the encoded threshold. Page poll interval reduced from 5s to 15s.

### Fixed

- **Reactive infinite loop on host detail page** — the post-rewrite host page had a `$effect` that read `bundle` for its guard while `load()` reassigned `bundle`, triggering re-runs at HTTP-roundtrip rate (~120 req/sec). Replaced with a `lastFetchKey`-guarded effect that depends only on `id` and `range`. Removed the duplicate `onMount` `load()` call since the effect now handles initial fetch. **Why:** caught in dev when build logs showed thousands of GETs in seconds; the fix also rules out the same pattern on the two other `$effect` sites (`Chart.svelte`, `+layout.svelte`), both of which were already properly guarded.

### Changed

- **Hub config interface** — `Hub.SetConfigPusher` and `Hub.SetLocalApplier` seams (interfaces declared locally to avoid import cycles). `cmd/hub/main.go` wires the agent handler as the pusher and the local collector as the applier, then calls `PushConfigToAgent` once at startup so persisted `host_config` survives hub restarts.
- **API `/api/alerts/metadata`** — adds `categories` (dotted-target families + leaves) and lists `shoutrrr` in `channel_types` so the rule editor can drive the new picker.
- **`SupportedOps`** — adds `==` and `!=` (for `host.status` and future container-state alerts). `ValidateRule` accepts dotted metrics, validates the category and the leaf suffix.

### Removed

- `internal/alerts/ch_discord.go`, `ch_slack.go`, `ch_ntfy.go`, `ch_gotify.go` — replaced by `ch_shoutrrr.go` + URL translators in the same file. Legacy config structs moved to `legacy_configs.go` so the API can still validate stored channel rows.
