...
**v0.2.0-alpha.5** — Phase 1 complete: remote agent transport, token-based auth, exponential-backoff reconnect, and the agent onboarding wizard (two-step: name → copy command with binary/Docker toggle, one-time token warning). **v0.2.0-alpha.4** — UI/UX quality-of-life pass: host status pills (online/stale/offline) on all host cards, per-host alert badges on the dashboard, network throughput in the dashboard footer row, stale/offline banners on the host detail page, quick container search, ESC-to-close for all modals, dynamic page titles, absolute-timestamp tooltips on all relative times, and a global toast notification system replacing bare `alert()` calls. **v0.2.0-alpha.3** added chart UX improvements (rich hover tooltip, drag-to-zoom, dynamic Y-axis sizing, 2-column chart grid) and extensible alert notification channels: Discord, Slack, ntfy, Gotify, and generic webhook, each with per-rule severity levels (info/warning/critical), per-channel minimum-severity filtering, and configurable resolve notifications. **v0.2.0-alpha.2** added full Beszel monitoring parity (historical per-interface/mount/IO charts, process list, memory breakdown). **v0.2.0-alpha.1** added rich live monitoring depth and complete container lifecycle. **v0.1.0** established the base foundation.
