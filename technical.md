...
| `hosts/[id]/+page.svelte` | Host detail. Fetches the historical range (`api.metrics`) and the live rich snapshot (`api.latest`) in parallel. Sub-navigation: Overview (active), Containers, Networks/Volumes/Images/Logs (placeholders at 0.45 opacity). Stat cards: CPU %, memory (GB used + %, bar), disk / (GB + %, bar), uptime. Rich live sections: per-core CPU grid (C0…Cn bars, color-coded at 75%/90%), network interfaces table (rx/tx rate + cumulative bytes, skip lo and veth*), disk mounts table (mount, device, fstype, used/total GB, bar), disk I/O table (device, read/write rate + totals), temperature grid (from `temps`). Historical charts with `fmtGiB` valueFormatter for memory/disk and `fmtBytesRate` for network. Auto-refresh every 5s. |
| `hosts/[id]/containers/+page.svelte` | Container management. Same sub-nav with Containers active. Filter controls (all / running / exited / paused) and sort controls (Name / State / CPU / Mem, ascending/descending toggle). `filtered()` derived state combines both. Row click expands an inline inspect panel (two-column layout): left column has Configuration (image, state, restart policy, cmd, ports, mounts, env expandable, labels); right column has Live stats (if running), Resource limits (view + inline edit: CPU cores → nanoCPUs × 1e9, memory GiB → bytes × 1073741824), and Actions (restart/stop/pause/unpause/recreate with confirm, force-remove, logs). Logs modal: full-width, 1000-line tail, filter input. Create modal: same surface form as before (image, name, restart policy, env, ports, volumes, auto-start). |
