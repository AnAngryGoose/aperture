...
| `hosts/[id]/+page.svelte` | Host detail. Fetches six things in parallel: host, aggregate metrics history, latest (rich live), net history, mount history, disk I/O history. Sub-navigation: Overview (active), Containers, Networks/Volumes/Images/Logs (placeholders). Stat cards: CPU %, memory (GB used + 3-segment used/cached/free bar), disk / (GB + %, bar), uptime. Rich live sections: per-core CPU grid (C0…Cn bars, color-coded at 75%/90%), network interfaces live table, disk mounts live table, disk I/O live table, temperature grid, **process table** (top 40 by CPU/mem, sort toggle). Historical chart sections: CPU, Memory, Disk (/), Network aggregate, Load average (existing); **per-interface network** (one chart per interface, rates from delta/elapsed); **per-mount disk** (one chart per mount, used/total GiB); **disk I/O per device** (one chart for all devices, read/write rates). `deriveRates(timestamps, bytes)` helper computes bytes/s from cumulative byte deltas. `memBreakdown` derived function builds the 3-segment percentages. `ifaceCharts`, `mountCharts`, `diskIOSeries` are derived states for the three new chart sections. Auto-refresh every 5s. |
